...
{
    public partial class ReporteVehiculosPeatonal : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                ReportDataSource rds = new ReportDataSource("DataSet2", dtPersonal);
                rds.Name = "DataSet2"; // Nombre del conjunto de datos en el informe
                rds.Value = dtPersonal;

                // Configurar el ReportViewer
                RpPersonal.LocalReport.DataSources.Clear();
                RpPersonal.LocalReport.DataSources.Add(rds);
                RpPersonal.LocalReport.ReportPath = Server.MapPath("Report2.rdlc"); // Ruta del informe (.rdlc)
